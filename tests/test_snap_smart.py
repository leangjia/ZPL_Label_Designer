# -*- coding: utf-8 -*-
"""УМНЫЙ тест с анализом логов для детекции проблем snap"""
...
class LogAnalyzer:
    ...
    @staticmethod
    def parse_final_pos_logs(stderr):
        ...
        before_matches = re.findall(before_pattern, stderr)
        after_matches = re.findall(after_pattern, stderr)
        saved_matches = re.findall(saved_pattern, stderr)
    ...
    def detect_snap_issues(snap_logs, final_logs):
        ...
        # Проверка 2: FINAL-POS Before должен совпадать с After если snap включен
...
def test_snap_with_log_analysis():
    """Тест с детальным анализом логов"""
    ...
    # Временно перенаправить stderr для захвата логов
    ...
    snap_logs = analyzer.parse_snap_logs(stderr_logs)
    final_logs = analyzer.parse_final_pos_logs(stderr_logs)
...
if __name__ == "__main__":
    sys.exit(test_snap_with_log_analysis())
